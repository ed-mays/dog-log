...
- **BR-10** — During an active incident, the system MUST surface a one-tap call action for the pet's vet IF at least one vet linked to the pet has a phone number on file; the action MUST resolve to the platform's `tel:` handler. If multiple vets are linked, selection rule is deferred to design phase (DQ added).
- **BR-11** — The call action MUST be hidden (not rendered, not disabled) when no vet phone number is on file for the pet.
...
- **OQ-7** — Open (added round 2 from cold-read S4-sub1). "Primary vet" selection rule when a pet is linked to multiple vets. Options: (a) add a `primary: boolean` flag on PetVet (data-model change), (b) most-recently-added wins, (c) most-recently-called wins. Resolves before tasks phase; touches the existing `petVetService` / `petVetRepository`.
