...
src/features/incidents/
  ...
  hooks/
    ...
    useIncidentHistory.ts               # per-pet incident list (BR-23) — added round 48
    useIncidentHistory.test.ts
...
- **2026-05-03 round 48** — Amended §D2 file map: added `useIncidentHistory.ts` + `.test.ts` to the `incidents/hooks/` listing. Surfaced by cold-reader MEDIUM scope-4 on the T-31 pushback dispatch (the per-pet history feature requires a hook to follow the project's Component → Hook → Service layering, but §D2's hook listing only enumerated `useActiveIncident.ts` + `useIncidentTimer.ts` from earlier slices). No spec change. Operator-applied this round (not via `harness arbitrate`) because the resolution was mechanical: add the file to the map.
