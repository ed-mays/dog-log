...
### `[x]` T-27 — ActivationPetPicker component
...
### `[x]` T-31 — IncidentHistoryList component

- **Cite:** spec BR-23, BR-24, BR-25; design §D2
- **What:** Create `src/features/incidents/components/IncidentHistoryList.tsx`. Takes `petId`. Queries `incidentService.listForPet(userId, petId)` (add this method — `userId` first to match the project's existing `IncidentService` convention). Per the project's layered architecture (Component → Hook → Service), introduce `useIncidentHistory(petId)` which reads `userId` from auth state and calls the service; the component consumes the hook (drop the `incidents` prop). Renders rows: start time, duration, type label or "untyped", severity or blank, journal one-line excerpt or blank. Tap → navigate to `/pets/:petId/incidents/:id`.
- **Verify:** Component test: AC-9 Given/When/Then. List excludes soft-deleted (filter in repository query, not component).
...
- **2026-05-03** — T-16: passed manual smoke (`pnpm run dev:with-emulators` → sign in → FAB → start → STOP → Firestore document persisted with `endedAt` set). Closes slice 1. One bug surfaced and fixed in PR #181 before T-16 passed: `useIncidentTimer` ignored `endedAt` and kept ticking after STOP; root cause was a §D2 post-STOP-invariant gap that propagated from the round-31 `amend_design` into the store but not down to the timer hook signature. Captured as harness lesson: per-task cold-reads do not catch invariant propagation across previously-shipped tasks. Watch slice 2 for similar leakage. Spec/design unchanged.
...
- **2026-05-03** — T-31: amended `What:` line. Original wording said `incidentService.listForPet(petId)` (1-arg) and was silent on the hook layer. Reconciled to `listForPet(userId, petId)` (matches the existing `IncidentService` convention — every other service method takes `userId` as first arg) plus explicit `useIncidentHistory(petId)` hook (matches project's Component → Hook → Service layering). Surfaced by cold-reader HIGH scope-7 on the round-48 pushback dispatch (builder shipped 2-arg + hook per project convention; cold-reader correctly flagged the divergence from the literal task contract). Operator-amended task (option 2b) rather than re-pushing back to drop `userId` from the public API (option 2a would have broken the 10-method-deep convention in the same file). **Methodology finding #18 (new): operator pushback can introduce its own spec divergence.** The round-48 operator pushback findings file (and the arbiter's verbatim pushback text) BOTH inferred "needs userId from auth" without checking the task contract literally; cold-reader pre-flight + scope-7 caught it on the next dispatch. Single instance (round 48); threshold = 2nd recurrence to act. The methodology held — pre-flight bounced the operator's own divergence as cleanly as it bounces the builder's. Spec unchanged. Slice 3 still at 4/10 (T-31 will mark `[x]` when its PR lands).
