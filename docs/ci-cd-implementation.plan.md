...
1. Create `dog-log-dev` and `dog-log-staging` projects; enable Hosting.
2. Add `firebase.json` and `.firebaserc` as shown.
3. Create `github-actions-deployer` service account in each project; upload their JSON keys to repo secrets.
4. Add GitHub Actions repo variables for DEV*\*/STG*\* `VITE_...` values.
